...
pub fn type_container(meta: &StoryMeta, blorb: bool) -> Option<&'static str> {
    match meta.engine {
        ...
        Engine::Glulx => None,
        // A ZX Spectrum snapshot names its container the way a Z-code row
        // names its floppy: the bytes lanthorn runs came out of a 48K memory
        // dump, not out of a `.dat` (SQ-1478). Read off the pictures rather
        // than off the filename, for the same reason `disk_image` is — the
        // mount decides, never the extension.
        Engine::Scott => match meta.scott_pictures {
            Some(ScottPictures::NativeZx { .. }) => Some("z80"),
            _ => blorb.then_some("blorb"),
        },
    }
}
