...
**§9.3's wording is the one runtime fact this loader forces that `crate::c64`'s does not**, and `Database` gained a field for it: `second_person`, set here and clear there, because "the wording follows the platform, not the series" — the Commodore 64 releases of the same eleven titles carry a first-person block in the file and §6.4 measured that not one second-person string occurs anywhere in them. `Vm::new_full` honours it exactly as it honours `mysterious` and `ti99`.

**Where the specification is wrong for these specimens**, recorded here because a spec's own errata belong beside the code that found them: §6.1's "every ZX release in the series stores exactly 82 messages" (only *Arrow of Death part 1* does; the eleven spread 65-99), §6.1's "their pictures … a room's picture number is the room number minus one" is right but §4.5's "word 0 is not a field" is not (it is the picture pointer), and §4.2's "up to 27 [all-NUL cells] in one dictionary" is 47. Everything else §4-§6 says about these releases held, including the two §4.2 rules that matter most — read the GRID, and do **not** apply the leading-NUL alignment escape on ZX.
