...
**SQ-1478 (the ZX Spectrum *Mysterious Adventures* now LOAD — and load with no catalogue at all, 2026-09-09):** the fifth encoding, and the first memory-image dialect that needs **nothing** per release. `crates/scott/src/zx_mysterious.rs` implements `docs/internals/scott-dialects-spec.md` §4.1-§4.6, §6, §7.1, §8.2 and §11 for Brian Howarth's eleven ZX releases, and `Database::parse` recognises a `.z80` snapshot, decompresses it (`crate::z80`, SQ-1452) and reads it — so ONE entry point answers for five encodings and the app needed no new loading wiring at all.

What made a catalogue unnecessary is a fact about the driver rather than about the format. §4.6 tells an implementer to "look for a driver pointer block before you tabulate anything", and this series has one **inside the header**: the nine little-endian words that follow the twelve counts are the addresses of the room descriptions, the connections, the item locations, the driver's working copy of them, the verb cells, the noun cells, the room descriptions again, the messages and the item descriptions, with a tenth (one past the item descriptions, where the picture block begins) in the word §4.5 calls the early shape's "unused" word 0. That turns both of §4.6's "two numbers, and no more" into derivations: the **field order** is found by trying §4.5's four candidate orders at every address in the 48K image and keeping the one whose counts make all ten spans come out exactly right — measured, exactly one (order, address) pair survives per title, and none at all in the nine other snapshots of the same archive — and the **verb/noun split** is simply read, because each block has its own pointer and holds exactly (word count + 1) cells. §4.6 calls that split "genuinely not derivable"; it is, on this platform.

The **stored action count** is checkable too, which is the one thing §5.3's repairs exist for elsewhere. The table's start is the connections less (count + 1) × 16, so any count lands it on the next table by construction; what pins the count is that every record it claims must carry a possible vocabulary word (verb × 150 + noun, below 150 × 150) **and** the sixteen bytes in front of the table must not — one too many and record 0 is the driver's code, one too few and the real record 0 sits in front of the table. Both hold in all eleven, so no ZX release needs §5.3's *Pulsar 7* repair, and a release that did would be detected rather than mis-read.

**§9.3's wording turned out not to be a fact this loader forces, and finding that out cost a field.** §6.4 tabulates a second-person set as "the ZX Spectrum wording" and §9.3 says to force it for a recognised ZX release, warning only against forcing it on the series — because §6.4 had already measured the Commodore 64 files carrying a first-person block of their own. `Database` therefore gained `second_person`, set here and clear there, and `Vm::new_full` honoured it. Then the same measurement was run on this platform: **every string of §6.4's second-person column, searched for in all eleven decompressed images, occurs nowhere; the first-person block is in all eleven**, around `$64C1`, tape prompts (`Get tape ready..Press <ENTER>`) and all. So the field was removed again, `parse_zx_mysterious` forces §9.2's two lamp options and nothing else, and `Options::you_are` stays the host's on both platforms. Appendix A item 16 of the specification carries the block string for string.

**Two wording bugs fell out of chasing it, and both are fixed.** `Presentation::C64` — the crate's DEFAULT layout, and what both lanthorn and `scott-cli` show — spelled `I'm in a ` and `I can also see:` as literals and never consulted `Wording` at all, so `Options::you_are` reached the death and inventory replies and stopped at the two lines a player reads every turn; it takes both from the table now, and the `!you_are` forms are the literals it carried, so nothing moves with the option off. And `Wording::room_prefix`'s second-person form was `"You are "` — the one string in that table no parity or golden case covered, and the one that was wrong, since every Scott room text in every dialect continues "in a": with it, *The Golden Baton*'s room 1 printed `You are dense forest, very SPOOKY`. §6.4's own table gives `I'm in a ` against `You are in a `, the first-person form already matched it, and now both do.

**Where the specification is wrong for these specimens**, recorded here because a spec's own errata belong beside the code that found them: §6.4's and §9.3's second person (above — the largest of them, and the one that would have shipped a wrong screen), §6.4's claim that the visible-objects heading is not in the ZX files (it is, at `$66BA`), §6.1's "every ZX release in the series stores exactly 82 messages" (only *Arrow of Death part 1* does; the eleven spread 65-99), §4.5's "word 0 is not a field" (it is the picture pointer on this platform), and §4.2's "up to 27 [all-NUL cells] in one dictionary" (47). Everything else §4-§6 says about these releases held, including the two §4.2 rules that matter most — read the GRID, and do **not** apply the leading-NUL alignment escape on ZX.

**The pictures are §8.2's, unchanged**: the same Family B display lists the Commodore 64 releases carry, at an address the pointer block gives, decoded by `crate::c64`'s own walker (`decode_family_b_lists_at_most`, the one addition — a Spectrum image is a fixed 48K with unused RAM after the block, where a program file simply ends, so the walk needs the room count as a bound). Only the palette differs: §8.2's optimised Sinclair table with an identity remap, against the Commodore 64's with remap table A. Drawing them in the app is not done — `picker`'s story-info panel reports them ("Pictures: native ZX Spectrum (vector, N rooms)") and the TYPE column names the container (`Scott (z80)`), but `ScottSession` has no ZX branch yet, so a `.z80` plays as text. That is the SQ-1463-shaped follow-up for this platform, and the decoder it needs is already public.
---

**SQ-1475 (2026-09-09): the US S.A.G.A. releases' own pictures — picture family C.** `crates/scott/src/saga_pictures.rs` implements `docs/internals/scott-dialects-spec.md` §8.3: `decode_family_c(record, platform)` reads the twelve-byte header (the "+3" biased left and right edges in 8-pixel columns, the top and bottom rows, four colour bytes), walks the byte-pair run-length stream, and paints column-major 8-pixel strips onto a fixed 280 x 160 indexed canvas with a four-entry palette whose entry 0 is black whatever the file says. `SagaPlatform` picks the colour table — §8.3's bare thirteen-colour Commodore 64 lookup, or the Atari's hardware palette, of which this implementation answers only the entries §8.3 actually states (the luminance row and the sixteen hand-substituted overrides) and reports the rest as unrecognised rather than reconstructing 224 colours from prose. `Picture::unrecognised_colours` is that channel in both directions, and two of the *Hulk*'s own records need it. `saga_us` gained the naming half — `parse_picture_file_name` / `is_picture_file_name` for §8.3's Commodore 64 rule and `picture_file_name` for its inverse — and `Vm::current_picture` now folds in §12.11's two release-specific rules (the *Hulk*'s five remapped room pairs, and darkness drawing picture index 0 rather than nothing), so a host asks one question and needs to know neither.

**Two corrections to §8.3, both found against an independent oracle and both load-bearing.** §8.3 says "when it passes the height", which reads naturally as an exclusive limit; the limit is **inclusive** on both axes, so a column takes `(bottom − top) / 2 + 1` byte pairs and its last pair paints rows `bottom` and `bottom + 1`. Read exclusively, every column falls two rows short of its data and the next starts two rows lower — the picture comes out sheared, and on the *Hulk*'s title screen that reads as an entirely plausible image of the Hulk with a legible `QUESTPROBE` wordmark, which is why the arithmetic alone could not settle it. The oracle that did is §10.1's own shape: *The Hulk* shipped the same seventy pictures on MS-DOS as family E (§8.5), whose storage order — row-major with a two-bank interleave — shares no arithmetic with family C at all, and decoding `R0199.PAK` beside `R01099` shows one composition and one wordmark. The second correction follows from the first: the nominal canvas is **280 x 160**, not §8.3's stated 158, which is also what §8.4 gives for the same artwork on the Apple II.

**In the app, family C is a third `PictSource`.** `PictSource::from_scott_saga` holds the raw records keyed by file name (about 90 KB for the *Hulk*, against ~3 MB of decoded pixels a player may never see) and `PictSource::get` resolves a picture NUMBER through `scott::picture_file_name` — no offset, unlike family B's `resnum - 1`, because §12.10's "a room's picture index IS the room number" has already been resolved by `Vm::current_picture`. The records have to arrive from outside the session for the same reason a Blorb does: they are separate files on the release disk (§12.10) and the mount does not outlive the load, so `hints::load_mounted_story_full` returns a `MountedStory` carrying them and `startup.rs` hands them to `ScottSession::new_with_options`; `@restart` re-reads them off the same path rather than carrying seventy records in app state. Nothing else in the band changed — same `PICTURE_ROWS`, same `GraphicsWindow`, same backend-neutral fit, no protocol branch.

**The Atari half is still out of reach, and for a reason the specification names.** §8.3 puts the Atari's pictures on the companion side at hard-coded (usage, index, offset) triples, and §12.10 says those lists "are not recoverable from the database — they are the one thing about these releases that must still be tabulated". Nothing pairs a S.A.G.A. side A with its side B either (`cli_host::disk_set` pairs by filename for the multi-disk Z-machine releases only). An Atari side A therefore reports "no pictures on this file" — deliberately distinct from a text-only game, which reports none at all — and a probe of the seven side Bs finds *Voodoo Castle*, *The Count* and *Claymorgue Castle* carrying records at the Commodore 64's own characteristic placements while the other four use different ones, so the tabulation is genuinely per-title.
