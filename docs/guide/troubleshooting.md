...
For anyone whose screen, sound or story isn't behaving — the things players
actually run into, and what to do about each.

## The map (or a border control) shows boxes or question marks instead of glyphs
...
correctly, then sets everything — arrows, portals, border controls — from
your answer; run it again any time with `/run-font-check` after changing
fonts. See [missing or corrupted glyphs](../internals/glyphs.md) for exactly
which Unicode blocks are involved, and for the one modern ask (diagonal corner stubs) that
...
It probably hasn't. A Glulx story whose turn runs for ten seconds without ever
...
- [Missing or corrupted glyphs](../internals/glyphs.md) — exactly which Unicode blocks your font needs
- [Platforms](../internals/platforms.md) — what differs on macOS, Linux and Windows, and why
